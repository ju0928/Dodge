...
{
    public GameObject bulletPrfab; //생성할 탄알의 원본 프리팹
    public float spawnRateMin = 0.5f; // 최소 생성 주기
    public float spawnRateMax = 3f; // 최대 생성 주기

    private Transform targt; //발사할 대상
    ...
    void Start()
    {
        ...
        timeAfterSpawn = 0f;
        //탄알 생성 간격을 spawnRateMin 사이에서 랜덤 지정
    ...
    void Update()
    {
        ...
        if (timeAfterSpawn >= spawnRate)
        {
            ...
            // vulletPrefab의 복제본을 transform.position 위치와 transform.rotation 회전으로 생성
            GameObject bullet
                = Instantiate(bulletPrfab, transform.position, transform.rotation);
            // 생성된 bullet 게임 오브젝트의 정면 방향이 target을 향하도록 회전
            bullet.transform.LookAt(targt);
            // 다음번 생성 간격을 spawnRaqteMi, spawnRateMax 사이에서 랜덤 지정
